...
- Composants Angular standalone exportés via `src/public-api.ts` — `IconComponent`, `FormFieldComponent`, `ConfirmDialogComponent`, `ToastComponent`, `PasswordStrengthComponent` + services `IconRegistry`, `ToastService`, `PasswordPolicyService` (selectors `pivot-ds-*`)
