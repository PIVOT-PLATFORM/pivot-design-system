...
- Composants Angular standalone exportés via `src/public-api.ts` — `IconComponent`, `FormFieldComponent`, `ButtonComponent`, `InputComponent`, `TextareaComponent`, `CheckboxComponent`, `RadioGroupComponent`, `SwitchComponent`, `ConfirmDialogComponent`, `ToastComponent`, `PasswordStrengthComponent` + services `IconRegistry`, `ToastService`, `PasswordPolicyService` (selectors `pivot-ds-*`)
